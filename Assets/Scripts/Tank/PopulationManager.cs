...
public class PopulationManager : MonoBehaviour
{
    ...
    public float[] fitnessTillNewTest = null;
    ...
    void Epoch()
    {
        ...
        if (generation > 1 && fitnessTillNewTest.Length > testIndex && avgFitness > fitnessTillNewTest[testIndex])
        {
            testIndex++;
        }
    }
    ...
    void FixedUpdate () 
	{
        ...
        for (int i = 0; i < Mathf.Clamp((float)(IterationCount / 100.0f) * 50, 1, 50); i++)
        {
            foreach (Tank t in populationGOs)
            {
                ...
                {
                    if (populationGOs[j] == t)
                    {
                        continue;
                    }

                    float distance = Vector3.Distance(populationGOs[j].transform.position, t.transform.position);
